...
export function ExtractionApp() {
  ...
  useEffect(() => {
    setDimensions(p.defaults); setMetrics(p.defaultMetrics); setFields({}); setOptions({}); setSelectedKeys([]); setResources([]); setIncremental(false); setResourceError(''); setFieldSearch(''); setResourceSearch(''); setAccountErrors([]); setSourceFields({}); setSourceOptions({});
    if (product === 'api') {
  ...
  async function extract() {
    ...
    try {
      const spec = { product, resource: '', start, end, dimensions, metrics, incremental, options: product === 'api' ? apiOptions : options, targets: selected.map(r => ({ connection_id: r.connection_id, resource: r.id, options: product === 'sheets' ? sourceOptions[resourceKey(r)] || {} : {} })) };
