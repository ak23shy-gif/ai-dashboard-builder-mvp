...
export function ExtractionApp() {
  ...
  return <div className="gx-app">
    ...
    <div className="gx-main"><header className="gx-topbar"><div>Workspace <ChevronRight size={13} /><strong>{view === 'history' ? 'Extraction history' : 'Data extraction'}</strong></div><span className="gx-local"><span /> Running locally</span></header>
      ...
      <section className="gx-panel gx-query-panel"><div className="gx-panel-title"><h2><span className="gx-section-number">3</span>Self-contained API endpoint</h2><button className="gx-button gx-secondary" disabled={!queryUrl} onClick={() => navigator.clipboard?.writeText(queryUrl).catch(() => {})}><Copy size={15} />Copy query</button></div><div className="gx-query-body"><p className="gx-muted"><Link2 size={14} />Use this complete URL in Power BI/Dataflow or any API client. It contains the API key, sources, fields, dates, filters, and options needed to fetch JSON directly later.</p><textarea readOnly value={queryUrl || (product === 'api' && !apiOptions.url.trim() ? 'Paste an API endpoint URL to generate a query.' : !queryKey ? 'Connect Google once to generate your local API key.' : 'Select at least one source to generate a query.')} /></div></section>
