...
function createBaseMarketingDashboard(prompt: string): DashboardConfig {
  const includeBookings = /booking|conversion/i.test(prompt);
  const includeBrand = /brand|company|nurtur|starberry|yomdel|tpj/i.test(prompt);
  const includeChannel = /channel|organic|paid|email|social|referral|direct/i.test(prompt) || !includeBrand;
...
function focusOnSessions(prompt: string): DashboardConfig {
  return validateDashboardConfig({
    ...createBaseMarketingDashboard(prompt),
...
export function generateDemoDashboard(prompt: string, currentDashboard?: DashboardConfig): DashboardConfig {
  ...
  const dashboard = validateDashboardConfig(currentDashboard || createBaseMarketingDashboard(prompt));
  ...
  return createBaseMarketingDashboard(prompt);
}
